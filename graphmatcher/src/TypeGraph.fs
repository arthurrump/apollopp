...
module TypeGraphAnnotation =
    let encode : Encoder<TypeGraphAnnotation> = function
    ...
    let decode : Decoder<TypeGraphAnnotation> =
        Decode.oneOf [
...
module TypeGraphEdge =
    let encode : Encoder<TypeGraphEdge> = function
        ...
        | Annotated annotation -> Encode.object [ "annotation", TypeGraphAnnotation.encode annotation ]

    let decode : Decoder<TypeGraphEdge> =
        Decode.oneOf [
            ...
            Decode.field "annotation" TypeGraphAnnotation.decode |> Decode.map Annotated
        ]

type TypeGraph<'node when 'node : comparison> = Graph<'node, TypeGraphEdge>

module TypeGraph = 
    let encode (nodeEncoder: Encoder<'node>) : Encoder<TypeGraph<'node>> = 
        Graph.encode nodeEncoder TypeGraphEdge.encode
    let decode (nodeDecoder: Decoder<'node>) : Decoder<TypeGraph<'node>> = 
        Graph.decode nodeDecoder TypeGraphEdge.decode
